...
public class StreamTransformerApp {
    ...
    public static void main(String args[]) {
        ...
        try (Ignite ignite = Ignition.start("DuckAirlines-server.xml")) {
            ignite.addCacheConfiguration(createFlightStreamCache());
            ...
            try (IgniteDataStreamer<String, FlightPlans> stmr = ignite.dataStreamer("FlightPlanStreamCache")) {
                // Allow data updates.
                stmr.allowOverwrite(true);

                //stmr.receiver((igniteCache, collection) -> );
                stmr.receiver(StreamTransformer.from(new CacheEntryProcessor<String, FlightPlans, Object>() {
                    @Override
                    public Object process(MutableEntry<String, FlightPlans> entry, Object... arguments) throws EntryProcessorException {
                        System.out.println(entry.toString());
                        FlightPlans val = entry.getValue();
                        if (val != null) {
                            val.setFlightStatus("Cancelled");
                            entry.setValue(val);
                        }
                        return null;
                    }
                }));
                getCancelledFlightList(flightPlansStreamCache, ignite, stmr);
            }
            finally {
    ...
    private static CacheConfiguration<String, FlightPlans> createFlightStreamCache() {
        CacheConfiguration<String, FlightPlans> ccfg = new CacheConfiguration<>("FlightPlanStreamCache");
